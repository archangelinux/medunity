...
import type { LocationFacility, LocationReport } from '@/lib/types';
...
export function ReportModal({ facility, onClose, onSubmit }: ReportModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
      ...
      <div className="relative slide-up w-full md:max-w-md bg-bg rounded-t-[var(--radius-lg)] md:rounded-[var(--radius-lg)] p-1.5">
        <Card>
          {/* Header */}
          ...
          {/* Strain level (for professionals) */}
          {reporterType === 'medical-professional' && (
            <div className="mb-4">
              <label className="text-[0.8125rem] font-medium text-text-secondary block mb-2">
          ...
          {/* Message */}
          <div className="mb-4">
            <label className="text-[0.8125rem] font-medium text-text-secondary block mb-2">Your update</label>
            <textarea
              value={message}
              onChange={(e) => setMessage(e.target.value)}
              placeholder="Share what you're seeing — wait times, conditions, availability..."
              rows={3}
              className="w-full px-3 py-2 rounded-[var(--radius-sm)] bg-surface-soft border border-border-soft
          ...
          {/* Submit */}
          <Button onClick={handleSubmit} disabled={!message.trim()} className="w-full" icon={<Send size={16} />}>
            Submit Report
          </Button>
        </Card>
      </div>
